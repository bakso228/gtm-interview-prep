...
import type { DayScore } from "@/lib/quiz-scores";
...
function yAt(pct: number): number {
  return P.top + (1 - pct / 100) * PH;
}

export function QuizDayChart({ days }: { days: DayScore[] }) {
  ...
  const n = data.length;
  const pts = data.map((d, i) => ({ ...d, x: xAt(i, n), y: yAt(d.percentage) }));
          ...
          <g key={p.date}>
            ...
            {isH && (
              <g>
                ...
                >
                  {p.percentage}% · {p.correct}/{p.total}
                </text>
              </g>
            )}
            {showLabel && (
